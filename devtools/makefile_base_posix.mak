...
LIB_END_EXE = -Wl,--end-group -lm -ldl -lstdc++ -lpthread

LIB_START_SHLIB = $(PATHWRAP) -static-libgcc -Wl,--start-group
LIB_END_SHLIB = -Wl,--end-group -lm -ldl -lstdc++ -lpthread -l:$(LD_SO) -Wl,--version-script=$(SRCROOT)/devtools/version_script.linux.txt
...
ifeq ($(BUILDING_MULTI_ARCH),1)
	...
	COMPILE_FILE_BASE = \
		mkdir -p $(OBJ_DIR) && \
		echo $(CXX) $(CXXFLAGS) $(GENDEP_CXXFLAGS) -o $@ -c $< >> compile_options.txt && \
...
ifneq "$(GAMEOUTPUTFILE)" ""
	$(QUIET_PREFIX) -rm -f $(GAMEOUTPUTFILE)$(SYM_EXT)"
...
$(GAMEOUTPUTFILE): $(OUTPUTFILE)
	...
	cp -v $(OUTPUTFILE) $(GAMEOUTPUTFILE) $(QUIET_ECHO_POSTFIX);
